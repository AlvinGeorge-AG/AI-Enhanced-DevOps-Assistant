...
# Keys where an empty Prometheus result means "unknown", not zero.
UNKNOWN_ON_EMPTY = {"active_replicas"}


class ContextBuilder:
    async def get_system_state(self) -> dict:
        queries = {
            "cpu_usage_percent": 'avg(rate(process_cpu_seconds_total{job="target_app"}[15s])) * 100',
            ...
            "active_alerts": 'count(ALERTS{alertstate=~"pending|firing"})',
        }
